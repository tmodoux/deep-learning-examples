...
# Making the test agent (won't update the model but will just use the shared trained model to explore)
def test(rank, params, shared_model):
    ...
    model.eval() # putting the model in "eval" model because its already trained
