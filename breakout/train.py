...
# Implementing a function to make sure the models share the same gradient, maybe overkill
...
def train(rank, params, shared_model, optimizer):
    torch.manual_seed(params.seed + rank) # shifting the seed with rank to desynchronize each training agent
    env = create_atari_env(params.env_name) # creating an optimized atari environment
    ...
    while True:
        ...
        for step in range(params.num_steps): # going through the num_steps exploration steps
            ...
            action = prob.multinomial().data # selecting an action by taking a random draw from the prob distribution (1x1 tensor)
            ...
            if done: # if the episode is done we reset the environment
                episode_length = 0
                state = env.reset()
            state = torch.from_numpy(state) # convert new state to tensor
        ...
        # in the end we have policy_loss = - sum_i log(pi_i)*gae + 0.01*H_i
