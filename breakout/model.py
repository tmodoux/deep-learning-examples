...
# Initializing and setting the variance of a tensor of weights => have different standard deviation on weights for Actor and Critic
def normalized_columns_initializer(weights, std=1.0):
    out = torch.randn(weights.size())
    out *= std / torch.sqrt(out.pow(2).sum(1).expand_as(out)) # allows to have variance(out) = std^2
    return out
...
class ActorCritic(torch.nn.Module):
    ...
    def __init__(self, num_inputs, action_space):
        ...
        # making an LSTM (Long Short Term Memory) to learn the temporal properties of the input - we obtain a big encoded vector S of size 256 that encodes an event of the game
        # why 32*3*3 ? use compute neurons function from previous project
        ...
        # small standard deviation of the weights for the actor and high for the critic => manage equilibrum between exploitation and exploration
        ...
        self.critic_linear.bias.data.fill_(0) # initializing the critic bias with zeros, should already be done via weights_init function
        # two types of bias in LSTM
    ...
    def forward(self, inputs):
        ...
        # also apply ELU (exponential linear unit) which is a more sophisticate RELU
